// ============================================
// App.js - Tail Me (CLEAN COMPOSER)
// ✅ 3-step composer flow
// ✅ Preset duration chips
// ✅ Adaptive single reveal input
// ✅ Theme moved to Settings tab
// ============================================
...
export default function App() {
  ...
  const [composerStep, setComposerStep] = useState(1); // 1 | 2 | 3
  ...
  // Reveal — single adaptive field
  const [revealKind, setRevealKind] = useState("coupon");
  const [revealValue, setRevealValue] = useState(""); // one field for all kinds

  // Duration presets
  const [durationPreset, setDurationPreset] = useState(2); // index into DURATION_PRESETS (default 24h)
  ...
  useEffect(() => {
    (async () => {
      try { await AsyncStorage.setItem("tailme_theme_pref", themePref); } catch {}
    })();
  }, [themePref]);
  ...
  const renderComposerStep = () => {
      ...
      <View style={{ gap: 14 }}>
        ...
        {composerMode === "private" && (
          <TextInput
            ...
            autoCapitalize="none"
            style={{
              backgroundColor: C.bg, borderWidth: 1, borderColor: C.brand,
    ...
    if (composerStep === 3) return (
      <View style={{ gap: 14 }}>
        ...
        >
          <Text style={{ color: composeIsAd ? C.amber : C.muted, fontWeight: "900" }}>
            Mark as Sponsored
          </Text>
          <View style={{
      ...
      <Modal visible={revealOpen} transparent animationType="fade" onRequestClose={() => setRevealOpen(false)} statusBarTranslucent>
        <Pressable onPress={() => setRevealOpen(false)} style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.6)", justifyContent: "center", padding: 14 }}>
          <Pressable onPress={(e) => e.stopPropagation()} style={{
            backgroundColor: C.panel, borderRadius: 22,
        ...
        <View style={{ flex: 1, padding: 24, justifyContent: "center" }}>
          ...
          <View style={{ backgroundColor: C.panel, borderRadius: 18, borderWidth: 1, borderColor: C.border, padding: 16 }}>
            ...
            <TextInput
              ...
              placeholder="alice" placeholderTextColor={C.dim}
              autoCapitalize="none"
      ...
      {screen === "hub" && me && (
        <View style={{ flex: 1, paddingBottom: 92 }}>
          <View style={{ paddingHorizontal: 16, paddingTop: 12 }}>
            <View style={{ flexDirection: "row", gap: 8 }}>
              <TouchableOpacity onPress={() => setTypeFilter("ALL")} style={{
                ...
                borderColor: typeFilter === "ALL" ? C.brand : C.border,
                backgroundColor: typeFilter === "ALL" ? "rgba(124,58,237,0.18)" : "transparent",
                alignItems: "center",
              }}>
                <Text style={{ color: typeFilter === "ALL" ? C.text : C.muted, fontWeight: "900", fontSize: 12 }}>ALL</Text>
              </TouchableOpacity>
              {TAIL_TYPES.map((t) => (
          ...
          <TailHome
            ...
            trending={applyTypeFilter(smartFeed.length > 0 ? smartFeed : publicTails).slice(0, 6)}
      ...
      {screen === "settings" && me && (
        <View style={{ flex: 1, padding: 16, paddingBottom: 92 }}>
          ...
          <View style={{ marginTop: 14, backgroundColor: C.panel, borderRadius: 18, borderWidth: 1, borderColor: C.border, padding: 16 }}>
            ...
            <TouchableOpacity onPress={() => { setMe(null); setScreen("login"); setPublicTails([]); setInboxTails([]); setSmartFeed([]); socket.disconnect(); }} style={{
              marginTop: 14, paddingVertical: 12, borderRadius: 14,
              borderWidth: 1, borderColor: C.red, alignItems: "center",
            }}>
          ...
          <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
            <View style={{ flex: 1, padding: 16, paddingBottom: 92 }}>
              ...
              <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
                <TextInput
                  value={chatText} onChangeText={setChatText}
                  placeholder="Type message…" placeholderTextColor={C.dim}
                  autoCapitalize="none"
      ...
      {me && screen !== "login" && (
        <>
          ...
          <View style={{
            ...
            paddingBottom: 14, paddingTop: 10, paddingHorizontal: 14,
            ...
            {[
              ...
              { key: "chat", label: "Chat", icon: "💬", disabled: !activeSession },
              ...
              return (
                <Pressable key={it.key}
                  onPress={() => disabled ? showToast("⚠️ Catch a tail first", 1500) : setScreen(it.key)}
                  style={{
                    flex: 1, paddingVertical: 8, borderRadius: 12, borderWidth: 1,
                    borderColor: active ? C.brand : C.border,
                    backgroundColor: active ? "rgba(124,58,237,0.18)" : "transparent",
                    alignItems: "center", opacity: disabled ? 0.4 : 1, gap: 2,
                  }}
                >
                  <Text style={{ fontSize: 14 }}>{it.icon}</Text>
                  <Text style={{ color: active ? C.text : C.muted, fontWeight: "900", fontSize: 10 }}>{it.label}</Text>
                </Pressable>
              );
            })}
          </View>
